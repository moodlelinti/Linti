...
<xsl:stylesheet
	...
	<xsl:for-each select="dim:field[@mdschema='dc' and @element='identifier' and @qualifier='uri']">
                <xsl:call-template name="fb-link">
    ...
    <xsl:template name="itemSummaryView-DIM-fields">
      ...
      <xsl:choose>
          ...
          <xsl:when test="$clause = 7 and (dim:field[@mdschema='informatica' and @element='distribucion' and @qualifier='licencia'])">
                    <tr class="ds-table-row {$phase}">
	                <td><span class="bold"><i18n:text>xmlui.dri2xhtml.METS-1.0.item-license</i18n:text>:</span></td>
	                <td>
		                <xsl:if test="'gpl' = (dim:field[@mdschema='informatica' and @element='distribucion' and @qualifier='licencia'])">
	                	    <span id="item-view-gpl">[imagen-gpl]&#160;<!--entidad espacio--></span>
				</xsl:if>

				<xsl:if test="'ccbysa' = (dim:field[@mdschema='informatica' and @element='distribucion' and @qualifier='licencia'])">
	                	    <span id="item-view-cc">[imagen-cc]&#160;<!--entidad espacio--></span>
				</xsl:if>
			</td>
	            </tr>
              <xsl:call-template name="itemSummaryView-DIM-fields">
          ...
          <xsl:when test="$clause = 8 and (dim:field[@mdschema='unlp-informatica' and @element='video' and @qualifier='uri'])">
                    <tr class="ds-table-row {$phase}">
                        <td><span class="bold"><i18n:text>Video en youtube</i18n:text>:</span></td>
                        <td>
                                <!-- Inserción del video youtube -->
                                <xsl:variable name="video-url" select="dim:field[@mdschema='unlp-informatica' and @element='video' and @qualifier='uri']"/>
    ...
    <xsl:template match="dim:dim" mode="itemDetailView-DIM">
        ...
        <span class="Z3999"><!--Duilio: cambio esto para corregir la vista completa del item-->
            <xsl:attribute name="title">
                 <xsl:call-template name="renderCOinS"/>
            </xsl:attribute>
        </span>
    ...
    <xsl:template match="dim:field" mode="itemDetailView-DIM">
            <tr>
                ...
                <td>
                    <i18n:text>
                        <!--<xsl:text>informatica.unlp.fullrecordview</xsl:text> -->
			<!--<xsl:text>unlp-informatica</xsl:text>-->
                        <!--<xsl:text>.</xsl:text>-->
        ...
        <tr>
            ...
            <td>
                <a>
                    ...
                    <xsl:attribute name="class">download-link</xsl:attribute>
            ...
            <td>
                <xsl:call-template name="dropbox-link">
                    <xsl:with-param name="url" select="mets:FLocat[@LOCTYPE='URL']/@xlink:href"/>
                </xsl:call-template>

            </td>
            <td>
                <xsl:call-template name="gdrive-link">
                    <xsl:with-param name="url" select="mets:FLocat[@LOCTYPE='URL']/@xlink:href"/>
                    <xsl:with-param name="filename" select="mets:FLocat[@LOCTYPE='URL']/@xlink:title"/>
                </xsl:call-template>
